...
func (j *JobManager) Start(ch *amqp.Channel) {
	...
	for qtdeProcess := 0; qtdeProcess < concurrency; qtdeProcess++ {
		go JobWorker(j.MessageChannel, j.JobReturnChannel, JobService, j.Domain, qtdeProcess)
	}
...
func (j *JobManager) checkParseErrors(jobResult JobWorkerResult) error {
	if jobResult.Job.ID != "" {
		log.Printf("MessageID #{jobResult.Message.DelivaryTag}, Error parsing job: #{jobResult.Job.ID}")
	} else {
		log.Printf("MessageID #{jobResult.Message.DelivaryTag}, Error parsing message: #{jobResult.Error}")
	}
